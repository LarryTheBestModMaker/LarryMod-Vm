...
class Scratch3SoundBlocks {
    ...
    getTimePosition (args, util) {
        ...
        console.log(soundBank.soundPlayers[soundId].currentTime)

        try {
            return Cast.toNumber(soundBank.soundPlayers[soundId].currentTime);
        } catch {
            return 0;
        }
    }
    ...
    _playSound (args, util, storeWaiting) {
        ...
        if (index >= 0) {
            ...
            if (sprite.soundBank) {
                ...
                setTimeout((function(soundThis, args, util) {
                    soundThis._playSoundTimer(args.SOUND_MENU, util)
                })(soundThis, args, util), 0)
    ...
    playSoundAllLolOpAOIUHFoiubea87fge87iufwhef87wye87fn (_, util) {
        ...
        for (let i = 0; i < sprite.sounds.length; i++) {
            const { soundId, name } = sprite.sounds[i];
            if (sprite.soundBank) {
                setTimeout((function(soundThis, name, util) {
                    soundThis._playSoundTimer(name, util)
                })(soundThis, name, util), 0)
